  ...
  return (
    <div className="min-h-screen bg-[#070b13] flex items-center justify-center p-6 font-sans relative">
      ...
      <div className="w-full max-w-4xl grid md:grid-cols-2 rounded-3xl overflow-hidden glass-panel border border-slate-800 shadow-2xl z-10">
        ...
        {/* Left column: Demo Pitch */}
        <div className="p-10 bg-slate-900/60 flex flex-col justify-between border-r border-slate-800/80">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-crimson-600 flex items-center justify-center shadow-lg">
              <Droplet className="w-5 h-5 text-white fill-white" />
            </div>
            <h2 className="text-lg font-bold text-white">HemoCast AI</h2>
          </div>

          <div className="space-y-6 py-10">
            <h3 className="text-2xl font-bold leading-tight text-white">
              The Predictive Blood Management Network.
            </h3>
            <p className="text-slate-400 text-sm leading-relaxed">
              Explore role-based control panels simulating real-world workflows for doctors, blood bank directors, and donors. 
            </p>
            <div className="space-y-3.5">
              <div className="flex gap-3 text-xs text-slate-300">
                <span className="w-1.5 h-1.5 rounded-full bg-crimson-500 mt-1.5"></span>
                <span><strong>Blood Banks:</strong> Monitor stock, run ML demand forecasts, schedule camps.</span>
              </div>
              <div className="flex gap-3 text-xs text-slate-300">
                <span className="w-1.5 h-1.5 rounded-full bg-crimson-500 mt-1.5"></span>
                <span><strong>Hospitals:</strong> File standard requests or trigger emergency SOS alerts.</span>
              </div>
              <div className="flex gap-3 text-xs text-slate-300">
                <span className="w-1.5 h-1.5 rounded-full bg-crimson-500 mt-1.5"></span>
                <span><strong>Donors:</strong> Track eligibility, streaks, and view localized shortage pings.</span>
              </div>
            </div>
          </div>

          <div className="flex items-center gap-2 text-xxs text-slate-500">
            <Award className="w-4 h-4 text-crimson-500" />
            <span>Built for ImpactForge Hackathon Evaluation</span>
          </div>
        </div>
          ...
          {isLogin && (
            <div className="mt-8 pt-6 border-t border-slate-900">
              <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-3 text-center">
                Judge Quick-Select Portals (No Password Required)
              </span>
